...
f_recode_macv <- function(st){
  ...
  clean_st <-  case_when(st %>% str_detect('CONG NHAN') ~ 'CONG NHAN',
                         ...
                         st %>% str_detect('THEU') ~ 'CONG NHAN',


                         st %>% str_detect('THO') ~ 'THO',
                         st %>% str_detect('MAI') ~ 'MAI',

                         st %>% str_detect('LAO DONG') ~ 'LDPT',
                         st %>% str_detect('LDPT') ~ 'LDPT',
                         st %>% str_detect('NANG NHOC') ~ 'LDPT',
                         st %>% str_detect('LD PHO THONG') ~ 'LDPT',
                         st %>% str_detect('KHAI THAC') ~ 'LDPT',
                         st %>% str_detect('DONG DINH') ~ 'LDPT',
                         st %>% str_detect('PHU VIEC') ~ 'LDPT',
                         st %>% str_detect('PHU KHO') ~ 'LDPT',
                         st %>% str_detect('PHUC VU') ~ 'LDPT',
                         st %>% str_detect('XUONG') ~ 'LDPT',

                         st %>% str_detect('TIEP VIEN') ~ 'TIEP VIEN',
                         st %>% str_detect('TONG DAI') ~ 'TONG DAI',

                         st %>% str_detect('NHAN VIEN') ~ 'NHAN VIEN',
                         st %>% str_detect('NHA VIEN') ~ 'NHAN VIEN',
                         st %>% str_detect('NV') ~ 'NHAN VIEN',
                         st %>% str_detect('N.V') ~ 'NHAN VIEN',
                         st %>% str_detect('TRUNG CAP') ~ 'NHAN VIEN',
                         st %>% str_detect('GDV') ~ 'GDV',
                         st %>% str_detect('GIAO DICH VIEN') ~ 'GDV',
                         st %>% str_detect('CHUYEN VIEN') ~ 'CHUYEN VIEN',
                         st %>% str_detect('NGHIEN CUU VIEN') ~ 'CHUYEN VIEN',
                         ...
                         st %>% str_detect('UY VIEN') ~ 'LANH DAO XA',
                         ...
                         st %>% str_detect('VAN THU') ~ 'CAN BO NHA NUOC',
                         ...
                         st %>% str_detect('TRUONG QUAY') ~ 'TRUONG QUAY',
                         ...
                         st %>% str_detect('DAI DIEN TIEU THU II') ~ 'KINH DOANH',
                         st %>% str_detect('GIAO HANG') ~ 'GIAO HANG',
                         st %>% str_detect('GIAO NHAN') ~ 'GIAO HANG',
                         ...
                         st %>% str_detect('K? THU?T') ~ 'KY THUAT',
                         ...
                         st %>% str_detect('PHO PHoNG') ~ 'PHO PHONG',
                         st %>% str_detect('PHO PHONG') ~ 'PHO PHONG',
                         st %>% str_detect('PHO TRUONG PHONG') ~ 'PHO PHONG',
